...
Now that you're authenticated, you'll be able to create a `CeramicDeveloper` profile of your own! Be sure to adjust the dummy queries below to meet your unique skill set.

<div style={{height: "60rem", width: "100%", margin: "auto"}}><MutationSandbox queries='mutations' /></div>

### Developer Feed

Below we've pulled in the most recent 8 `CeramicDev` records in addition to all verifications that have been issued in relation to that `CeramicDev` profile (these have been filtered to ensure each verification is from a unique profile):
...
Notice how some of the profiles have obtained attestations? We've intentionally omitted the ability to create an `AttestToDev` instance using this sandbox. You can attest to your own profile as well by cloning a local version of the application:
