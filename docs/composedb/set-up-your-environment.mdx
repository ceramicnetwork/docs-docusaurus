...
local machine using two simple steps:
...
:::note
By default, the command above will spin off a node which will connect to a `clay-testnet`. You can change this behaviour by providing a `--network` flag and specifying a network of your choice. For example:
