...
### Prometheus endpoint
In the `metrics` section of the daemon config, set the `prometheus` field to `true`:
...
```bash
COLLECTOR_HOSTNAME=<OPENTELEMETRY_COLLECTION_ENDPOINT>
CERAMIC_METRICS_EXPORTER_ENABLED=true
```
