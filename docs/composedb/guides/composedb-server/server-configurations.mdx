...
Each network has its own string designation. Find more information about the networks [here](../../../protocol/core/networking/networks).
...
### Persistent Storage
To run a Ceramic node in production, it is critical to persist the [Ceramic state store](../../../protocol/core/guides/ceramic-nodes/js-ceramic/running-cloud#ceramic-state-store) and the [IPFS datastore](https://github.com/ipfs/go-ipfs/blob/master/docs/config.md#datastorespec). The form of storage you choose should also be configured for disaster recovery with data redundancy, and some form of snapshotting and/or backups.
...
You can also use AWS S3 for data storage which is supported for both Ceramic and IPFS. Examples of the configuration are shared on the Ceramic docs [here](../../../protocol/core/guides/ceramic-nodes/js-ceramic/running-cloud#example-aws-s3-policies).
