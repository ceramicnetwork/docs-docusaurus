...
Sorting allows you to sort the returned documents by a specified order - ascending, specified as `ASC`, or descending, specified as `DESC`. For a field to be used for sorting the returned documents, it needs to have an index created just like we specified in [Create Indices](#create-indices) section of this guide.
